CLASS zcl_soar_manager DEFINITION
  ...
  PRIVATE SECTION.
    ...
    TYPES ty_table_inhousedev_gsrp TYPE HASHED TABLE OF zsoar_inhousedev WITH UNIQUE KEY class_name.
    ...
    DATA table_inhousedev_gsrp TYPE ty_table_inhousedev_gsrp.
...
CLASS zcl_soar_manager IMPLEMENTATION.
  ...
  METHOD zif_soar_manager~create.
    ...
    soar_manager->classes = driver->get_classes( ).

    SELECT *
        FROM zsoar_inhousedev
        INTO TABLE @soar_manager->table_inhousedev_gsrp.
...
*        DATA(soar_driver) = VALUE ty_ref_to_zif_soar_driver( ).
        DATA(inhousedev_gsrp) = REF #( table_inhousedev_gsrp[ class_name = public_class_name
                                                              inactive   = abap_false ] OPTIONAL ).
        IF inhousedev_gsrp IS BOUND.

*          READ REPORT inhousedev_gsrp->subroutine_pool_name INTO abap_source_code.
          DATA(srpoo) = lcl_soar_srpoo=>create( inhousedev_gsrp->subroutine_pool_name ).
          TRY.
              DATA(instance) = driver->instantiate( gensrp            = srpoo
...
*            result = soar_driver.
          ...
          TRY.
            ...
            CATCH cx_sy_move_cast_error INTO error.
              RAISE EXCEPTION NEW zcx_soar( text = 'Programming error: receiving variable type does not implement interface ZIF_XXXXXXXXX of &1. Please contact the support.'(006) msgv1 = public_class_name ).
            CATCH cx_root INTO error.
